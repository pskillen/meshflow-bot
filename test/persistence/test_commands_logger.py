...
class TestSqliteCommandLogger(unittest.TestCase):
    ...
    def test_initialize_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='unknown_requests'"
            )
            self.assertIsNotNone(cursor.fetchone())
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='responder_log'"
            )
            self.assertIsNotNone(cursor.fetchone())

    def test_log_command(self):
        command_instance = MagicMock(spec=AbstractCommand)
        command_instance.get_command_for_logging.return_value = (
            "base_cmd",
            ["sub_cmd1", "sub_cmd2"],
            "arg1 arg2",
        )
    ...
    def test_log_responder_handled(self):
        ...
        self.logger.log_responder_handled(
            "sender1", responder_instance, "response message"
        )
