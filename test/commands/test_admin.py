...
class TestAdminCommand(CommandWSCTestCase):
    ...
    def _setup_nodes_mock_data(self, node_list: list[MeshNode]):
        ...
        self.bot.command_logger.get_command_history = MagicMock(
            return_value=self.mock_command_history
        )
        self.bot.command_logger.get_unknown_command_history = MagicMock(
            return_value=self.mock_unknown_command_history
        )
        self.bot.command_logger.get_responder_history = MagicMock(
            return_value=self.mock_responder_history
        )

    def test_handle_packet_not_authorized(self):
        msg = build_test_text_message(
            "!admin", self.test_non_admin_nodes[0].user.id, self.bot.my_id
        )
    ...
    def test_handle_packet_authorized(self):
        msg = build_test_text_message(
            "!admin", self.test_admin_nodes[0].user.id, self.bot.my_id
        )
        self.command.handle_packet(msg)
        self.assert_message_sent(
    ...
    def test_reset_packets(self):
        msg = build_test_text_message(
            "!admin reset packets", self.test_admin_nodes[0].user.id, self.bot.my_id
        )
        self.command.handle_packet(msg)
    ...
    def test_reset_packets_missing_argument(self):
        msg = build_test_text_message(
            "!admin reset", self.test_admin_nodes[0].user.id, self.bot.my_id
        )
        self.command.handle_packet(msg)
        self.assert_message_sent(
    ...
    def test_reset_packets_unknown_argument(self):
        msg = build_test_text_message(
            "!admin reset unknown", self.test_admin_nodes[0].user.id, self.bot.my_id
        )
        self.command.handle_packet(msg)
        self.assert_message_sent(
    ...
    def test_show_users_user_not_found(self):
        msg = build_test_text_message(
            "!admin users !ffffff", self.test_admin_nodes[0].user.id, self.bot.my_id
        )
        self.command.handle_packet(msg)
    ...
    def test_show_users_user_found(self):
        ...
        midnight_7_days_ago = midnight_7_days_ago.replace(
        ...
        summary_response = (
            f"{target_node.user.long_name} - 2 cmds, 2 responders, 1 unknown cmds\n"
        )
        summary_response += f"Since {midnight_7_days_ago.strftime('%Y-%m-%d %H:%M:%S')}"
        self.assert_message_sent(
            summary_response, self.test_admin_nodes[0], multi_response=True
        )
        ...
        self.assert_message_sent(
            known_commands_response, self.test_admin_nodes[0], multi_response=True
        )

        unknown_commands_response = "Unknown Commands:\n"
        unknown_commands_response += "- unknown1\n"
        self.assert_message_sent(
            unknown_commands_response, self.test_admin_nodes[0], multi_response=True
        )
        ...
        self.assert_message_sent(
            responders_response, self.test_admin_nodes[0], multi_response=True
        )

    def test_show_users_all_users(self):
        msg = build_test_text_message(
            "!admin users", self.test_admin_nodes[0].user.id, self.bot.my_id
        )
        self.command.handle_packet(msg)
    ...
    def test_show_help(self):
        msg = build_test_text_message(
            "!admin help", self.test_admin_nodes[0].user.id, self.bot.my_id
        )
