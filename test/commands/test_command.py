...
class TestAbstractCommand(CommandTestCase):
    ...
    def test_handle_packet(self):
        msg = build_test_text_message(
            "!test", self.test_nodes[1].user.id, self.bot.my_id
        )
...
class TestAbstractCommandWithSubcommands(CommandWSCTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.command = ConcreteCommandWithSubcommands(
            bot=self.bot, base_command_str="test"
        )
    ...
    def test_show_help(self):
        msg = build_test_text_message(
            "!test help", self.test_nodes[1].user.id, self.bot.my_id
        )
        self.command.handle_packet(msg)
    ...
    def test_unknown_subcommand(self):
        msg = build_test_text_message(
            "!test unknown", self.test_nodes[1].user.id, self.bot.my_id
        )
        self.command.handle_packet(msg)
