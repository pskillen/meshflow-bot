...
class TestHelpCommand(CommandWSCTestCase):
    ...
    def test_handle_packet_no_additional_message(self):
        message = build_test_text_message(
            "!help", self.test_nodes[1].user.id, self.bot.my_id
        )
    ...
    def test_handle_packet_hello_command(self):
        message = build_test_text_message(
            "!help hello", self.test_nodes[1].user.id, self.bot.my_id
        )
        self.command.handle_packet(message)
    ...
    def test_handle_packet_ping_command(self):
        message = build_test_text_message(
            "!help ping", self.test_nodes[1].user.id, self.bot.my_id
        )
        self.command.handle_packet(message)
    ...
    def test_handle_packet_help_command(self):
        message = build_test_text_message(
            "!help help", self.test_nodes[1].user.id, self.bot.my_id
        )
        self.command.handle_packet(message)
    ...
    def test_handle_packet_unknown_command(self):
        message = build_test_text_message(
            "!help unknown", self.test_nodes[1].user.id, self.bot.my_id
        )
        self.command.handle_packet(message)
    ...
    def test_handle_packet_ping_with_and_without_exclamation(self):
        with_excl = build_test_text_message(
            "!help !ping", self.test_nodes[1].user.id, self.bot.my_id
        )
        without_excl = build_test_text_message(
            "!help ping", self.test_nodes[1].user.id, self.bot.my_id
        )
