...
class AbstractCommandLogger(abc.ABC):
    ...
    @abc.abstractmethod
    def log_responder_handled(
        self, sender_id: str, responder_instance, message_text: str
    ) -> None:
        pass

    @abc.abstractmethod
    def get_command_history(
        self, since: datetime, sender_id: str = None
    ) -> list[dict[str, Any]]:
        pass

    @abc.abstractmethod
    def get_unknown_command_history(
        self, since: datetime, sender_id: str = None
    ) -> list[dict[str, Any]]:
        pass
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    ...
                    message_text,
                    datetime.now(timezone.utc).isoformat(),
                    responder_instance.__class__.__name__,
                ),
            )
            conn.commit()
