...
class AbstractNodeDB(abc.ABC):
    ...
    ) -> list[MeshNode.Position]:
        pass

    @abc.abstractmethod
    def store_device_metrics(
        self, node_id: str, device_metrics: MeshNode.DeviceMetrics
    ):
        pass
    ...
    @abc.abstractmethod
    def get_device_metrics_log(
        self, node_id: str, start: datetime, end: datetime
    ) -> list[MeshNode.DeviceMetrics]:
        pass
...
class InMemoryNodeDB(AbstractNodeDB):
    ...
    def get_position_log(
        ...
        if node_id in self.positions:
            return [
                pos
                for pos in self.positions[node_id]
                if start <= pos.logged_time <= end
            ]
        return []
    ...
    def get_device_metrics_log(
        ...
        if node_id in self.device_metrics:
            return [
                metrics
                for metrics in self.device_metrics[node_id]
                if start <= metrics.logged_time <= end
            ]
        return []
...
class SqliteNodeDB(BaseSqlitePersistenceStore, AbstractNodeDB):
    ...
    def get_by_radio_id(self, node_id: str) -> MeshNode.User | None:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, short_name, long_name, macaddr, hw_model, public_key FROM nodes WHERE id = ?",
                (node_id,),
            )
            row = cursor.fetchone()
            if row:
                return MeshNode.User(
                    node_id=row[0],
                    short_name=row[1],
                    long_name=row[2],
                    macaddr=row[3],
    ...
    def list_nodes(self) -> list[MeshNode.User]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, short_name, long_name, macaddr, hw_model, public_key FROM nodes"
            )
